...
public class ChannelReleaseController {
    ...
    public ApiResponse<ReleaseResponse> currentRelease(@PathVariable UUID channelId) {
        ...
        return ApiResponse.success(ReleaseResponse.from(current));
    }

    /** Rolls the channel back to its previous release, marking the current one ROLLED_BACK. */
    @PostMapping("/rollback")
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ReleaseResponse> rollback(
            @PathVariable UUID channelId, @RequestHeader(value = "X-Actor", required = false) String actor) {
        return ApiResponse.success(ReleaseResponse.from(releases.rollbackChannel(channelId, actor)));
    }

    /** Returns the release history for the channel, newest first. */
    @GetMapping("/releases")
    public ApiResponse<List<ReleaseResponse>> channelReleases(@PathVariable UUID channelId) {
        return ApiResponse.success(releases.listChannelReleases(channelId).stream()
