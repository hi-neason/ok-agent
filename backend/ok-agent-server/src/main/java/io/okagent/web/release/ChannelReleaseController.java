...
public class ChannelReleaseController {
    ...
    public ReleaseResponse currentRelease(@PathVariable UUID channelId) {
        ...
        return ReleaseResponse.from(current);
    }
    ...
    public ReleaseResponse rollback(
        ...
        return ReleaseResponse.from(releases.rollbackChannel(channelId, actor));
    }

    /** Returns the release history for the channel, newest first. */
    @GetMapping("/releases")
    public List<ReleaseResponse> channelReleases(@PathVariable UUID channelId) {
        return releases.listChannelReleases(channelId).stream().map(ReleaseResponse::from).toList();
    }
}
